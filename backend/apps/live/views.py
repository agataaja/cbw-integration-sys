...
from apps.arena.models import ArenaClient, ArenaFight, ArenaFighter, ArenaSportEvent
...
class TunnelRegisterAPIView(APIView):
    ...
    def post(self, request):
        ...
        if data["status"] == Tunnel.Status.OFFLINE:
            return Response(
                {
                    "message": "Tunnel is offline, no update performed"
                },
                status=status.HTTP_200_OK,
            )

        arena_client = ArenaClient.objects.get(pk=1)
        ...
        return Response(
            {
                "message": "Tunnel created" if created else "Tunnel updated"
            },
            status=status.HTTP_200_OK,
        )